-module(name_server_b).
-export([start/1, loop/1, stop/0, status/0]). % Server exports.
-export([resolve/1, register/2, unregister/1]). % Client exports.
...
loop(NameIps) ->
  receive
    ...
    % Stops the server.
    {From, Tag, stop} ->
      reply_to(From, Tag, {ok, stopped});

    % Inquires the server status.
    {From, Tag, status} ->
      reply_to(From, Tag, {ok, length(NameIps)}),
      loop(NameIps);
    ...
    {From, Tag, {resolve, Name}} ->
      case lists:keyfind(Name, 1, NameIps) of
        false ->
          ...
          reply_to(From, Tag, {error, not_found});
        {Name, Ip} ->

          % Requested name was found - reply with OK.
          reply_to(From, Tag, {ok, Ip})
      end,
      loop(NameIps);
    ...
    {From, Tag, {register, Pair = {Name, Ip}}} ->
      case lists:keyfind(Name, 1, NameIps) of
        ...
        _ ->
          ...
          reply_to(From, Tag, {error, already_exists}),
          loop(NameIps)
      end;
    ...
    {From, Tag, {unregister, Name}} ->
      case lists:keyfind(Name, 1, NameIps) of
        ...
        {Name, Ip} ->
          ...
          % Name to be removed was found - reply to client with OK and removed
...
% Sends the reply to the specified client using the tag.
reply_to(To, Tag, Reply) -> To ! {Tag, Reply}.
