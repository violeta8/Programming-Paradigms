...
-module(server_gen).
-include("log.hrl").
-export([start/2, start_link/2, stop/1, init/2, loop/2]). % Server exports.
...
%    and the arguments 'Args'. It should return the 'State' that is used to
%    bootstrap the server (see below).
% 2. the terminate/1 implementation should accept a single parameter, the server
%    state  'State', and should return the status that is to be sent to the
...
%    the 'NewState' to be saved in the server. 'NewState' can either be equal to
%    the old state (in nothing was changed by the handle clause) or the actual
%    modifed state.
start(Mod, Args) ->
