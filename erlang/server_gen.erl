-module(server_gen).
-export([start/2, stop/1, init/2, loop/2]). % Server exports.
-export([rpc/2]). % Utility exports.

% Starts the server.
% Note: a new process is spawned, passing to the the module name Mod and the
% data that is to be used to initialise the server.
...
%    and the server state State. It should return only the new state (see
%    below).
% 2. the terminate/1 implementation should accept a single parameter State,
%    and should return the status that is to be sent to the client when the
%    server is stopped.
...
%    different type of request sent by the client (see name_server_gen for an
...
%    the NewState to be saved in the server. NewState can either be equal to the
%    old state (in nothing was changed by the handle clause) or the actual
%    modifed state.
start(Mod, State) ->
  spawn(?MODULE, init, [Mod, State]).
...
% that implements init/1 (e.g. name_server_gen is one such module).
% The argument State passed to the implementation of init/1 can choose to
% return the same State, or else modify it, according to one's needs. The
% returned state is used to call the process loop/2 below.
init(Mod, State) ->
  NewState = Mod:init(State),
  loop(Mod, NewState).

% The main process loop.
% Note: this function is called by init/1 above to service client messages.
% There are two types of messages the receive statement handles:
...
%    clause calls the  Mod:terminate/1 function to allow the implementor to send
%    a custom stop status.
